...
namespace Forminator\PayPal\Core;

use Forminator\PayPal\Log\PayPalLogFactory;
...
class PayPalLoggingManager
{
    ...
    private function __construct($loggerName)
    {
        ...
        $factory = array_key_exists('log.AdapterFactory', $config) && in_array('Forminator\PayPal\Log\PayPalLogFactory', class_implements($config['log.AdapterFactory'])) ? $config['log.AdapterFactory'] : '\Forminator\PayPal\Log\PayPalDefaultLogFactory';
