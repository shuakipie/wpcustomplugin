	...
	public function save_report_settings() {
		...
		if ( isset( $data['report-recipients'] ) && (bool) $data['scheduled-reports'] && ! $recipients_updated ) {
			if ( isset( $data['threshold'] ) ) {
				...
				$notice = sprintf(
					...
					esc_html__( 'Your changes have been saved successfully. You will get scheduled reports every %s.', 'wphb' ),
					esc_html( $freq[ $data['report-frequency'] ] )
				);
			}
		}

		$number_of_recipients = isset( $data['report-recipients'] ) ? count( $data['report-recipients'] ) : 0;
		...
		wp_send_json_success(
			array(
				...
				'notice'          => $notice,
				'recipientNotice' => Admin\Pro_Admin::get_reporting_message( ucfirst( $module ), $data['report-frequency'], $data['report-day'], $number_of_recipients ),
			)
		);
	}
