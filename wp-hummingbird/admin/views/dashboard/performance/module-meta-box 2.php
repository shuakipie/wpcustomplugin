		...
		<table class="sui-table">
			...
			<?php foreach ( $report->metrics as $rule => $rule_result ) : ?>
				...
				<tr class="wphb-performance-report-item" data-performance-url="<?php echo esc_attr( $perf_link . '#' . $rule ); ?>">
					...
					<td>
						<div class="sui-circle-score sui-grade-<?php echo esc_attr( \Hummingbird\Core\Modules\Performance::get_impact_class( absint( $score * 100 ) ) ); ?> sui-tooltip" data-tooltip="<?php echo absint( $score * 100 ); ?>/100" data-score="<?php echo absint( $score * 100 ); ?>"></div>
					</td>
					<td>
						<span><?php echo isset( $rule_result->displayValue ) ? esc_html( $rule_result->displayValue ) : esc_html__( 'N/A', 'wphb' ); ?></span>
						<i aria-hidden="true" class="sui-icon-<?php echo esc_attr( \Hummingbird\Core\Modules\Performance::get_impact_class( absint( $score * 100 ), 'icon' ) ); ?> sui-<?php echo esc_attr( \Hummingbird\Core\Modules\Performance::get_impact_class( absint( $score * 100 ) ) ); ?> sui-md"></i>
					</td>
				</tr>
			<?php endforeach; ?>
		</table>
		...
		<ul class="sui-list">
			...
			<li class="wphb-performance-report-item" data-performance-url="<?php echo esc_attr( $perf_link . '#wphb-box-performance-audits-opportunities' ); ?>">
				...
				<span class="sui-list-detail">
					<span class="sui-tag sui-tag-<?php echo esc_attr( \Hummingbird\Core\Modules\Performance::get_audits_class( $report->audits->opportunities ) ); ?>">
			...
			<li class="wphb-performance-report-item" data-performance-url="<?php echo esc_attr( $perf_link . '#wphb-box-performance-audits-diagnostics' ); ?>">
				...
				<span class="sui-list-detail">
					<span class="sui-tag sui-tag-<?php echo esc_attr( \Hummingbird\Core\Modules\Performance::get_audits_class( $report->audits->diagnostics ) ); ?>">
